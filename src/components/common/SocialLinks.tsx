...
export interface SocialLinkItem {
  id: "facebook" | "instagram" | "linkedin" | "whatsapp";
  name: string;
  href: string;
  ariaLabel: string;
}
        ...
        return (
          <a
            ...
            className={
              itemClassName ||
              (isCircle
                ? "w-9 h-9 flex items-center justify-center rounded-full bg-slate-100 text-slate-700 hover:text-[#024AE5] hover:bg-slate-200 transition-colors"
                : "inline-flex items-center justify-center text-slate-800 hover:text-[#024AE5] transition-colors p-1")
            }
          >
            <SocialIcon
              id={link.id}
              className={iconClassName || (isCircle ? "w-4 h-4 fill-current" : "w-4 h-4 fill-current")}
            />
          </a>
        );
      })}
